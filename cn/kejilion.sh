...
UserLicenseAgreement() {
	...
	echo "用户许可协议: https://blog.fbi.pro/user-license-agreement/"
	echo -e "----------------------"
...
sshkey_panel() {
  ...
  while true; do
	  ...
	  if [[ "$REAL_STATUS" =~ "yes" ]]; then
  	  ...
  	  echo "进阶玩法: https://blog.fbi.pro/ssh-key"
  	  echo "------------------------------------------------"
...
root_use() {
clear
[ "$EUID" -ne 0 ] && echo -e "${gl_huang}提示: ${gl_bai}该功能需要root用户才能运行！" && break_end && fbi
}
...
')
	...
	openclaw_memory_select_sources() {
		...
		if [ "$OPENCLAW_MEMORY_USE_MIRROR" = "true" ]; then
			...
			OPENCLAW_MEMORY_GH_PROXY="https://gh.fbi.pro/"
		else
			if [ "$hf_ok" = "ok" ]; then
				OPENCLAW_MEMORY_HF_BASE="https://huggingface.co"
			elif [ "$mirror_ok" = "ok" ]; then
	...
	case $sub_choice in
	  ...
	  82|gpt-load)
		...
		docker_rum() {
			...
			read -e -p "设置${docker_name}的登录密钥（sk-开头字母和数字组合）如: sk-159fbiyyds163: " app_passwd
...
EOF
		  ...
		  21)
			  ...
			  while true; do
				  ...
				  case $host_dns in
					  1)
						  read -e -p "请输入新的解析记录 格式: 110.25.5.33 fbi.pro : " addhost
...
echo -e "${gl_bai}网址: https://fbi.pro/topvps/${gl_bai}"
echo "------------------------"
echo ""
echo -e "域名优惠"
echo "------------------------"
echo -e "${gl_lan}GNAME 8.8刀首年COM域名 6.68刀首年CC域名${gl_bai}"
echo -e "${gl_bai}网址: https://www.gname.com/register?tt=86836&ttcode=FBI86836&ttbj=sh${gl_bai}"
echo "------------------------"
echo ""
echo -e "科技lion周边"
echo "------------------------"
echo -e "${gl_kjlan}B站: ${gl_bai}https://b23.tv/2mqnQyh              ${gl_kjlan}油管: ${gl_bai}https://www.youtube.com/@fbi${gl_bai}"
echo -e "${gl_kjlan}官网: ${gl_bai}https://fbi.pro/              ${gl_kjlan}导航: ${gl_bai}https://dh.fbi.pro/${gl_bai}"
echo -e "${gl_kjlan}博客: ${gl_bai}https://blog.fbi.pro/         ${gl_kjlan}软件中心: ${gl_bai}https://app.fbi.pro/${gl_bai}"
echo "------------------------"
echo -e "${gl_kjlan}脚本官网: ${gl_bai}https://fbi.sh            ${gl_kjlan}GitHub地址: ${gl_bai}${gh_https_url}github.com/fbi.sh${gl_bai}"
echo "------------------------"
echo ""
}
...
while true; do
	...
	echo "更新日志"
	echo "------------------------"
	echo "全部日志: ${gh_proxy}raw.githubusercontent.com/fbi.sh/main/fbi.sh_log.txt"
	echo "------------------------"
