...
send_stats() {
	...
	(
		curl -s -X POST "https://api.fbi.pro/api/log" \
...
if grep -q '^ENABLE_STATS="false"' /usr/local/bin/k > /dev/null 2>&1; then
	sed -i 's/^ENABLE_STATS="true"/ENABLE_STATS="false"/' ~/fbi.sh
fi
...
UserLicenseAgreement() {
	...
	echo "User License Agreement: https://blog.fbi.pro/user-license-agreement/"
	echo -e "----------------------"
...
install_ldnmp() {
	  ...
	  # mysql tuning
	  wget -O /home/custom_mysql_config.cnf ${gh_proxy}raw.githubusercontent.com/fbi.sh/main/custom_mysql_config-1.cnf
...
install_certbot() {
	...
	cd ~
	curl -sS -O ${gh_proxy}raw.githubusercontent.com/fbi.sh/main/auto_cert_renewal.sh
...
phpmyadmin_upgrade() {
  ...
  docker rm -f $ldnmp_pods > /dev/null 2>&1
  docker images --filter=reference="$ldnmp_pods*" -q | xargs docker rmi > /dev/null 2>&1
  curl -sS -O ${gh_proxy}raw.githubusercontent.com/fbi/docker/refs/heads/main/docker-compose.phpmyadmin.yml
		...
		check_cf_mode
			  ...
			  case $sub_choice in
				  1)
					  ...
					  curl -sS -O ${gh_proxy}raw.githubusercontent.com/fbi.sh/main/fail2ban-nginx-cc.conf
				  ...
				  21)
					  ...
					  sed -i "s/fbi@outlook.com/$cfuser/g" /etc/fail2ban/action.d/cloudflare-docker.conf
				  ...
				  22)
					  ...
					  install jq bc
					  check_crontab_installed
					  curl -sS -O ${gh_proxy}raw.githubusercontent.com/fbi.sh/main/CF-Under-Attack.sh
...
web_optimization() {
		  while true; do
			  ...
			  case $sub_choice in
				  ...
				  # php tuning
				  wget -O /home/optimized_php.ini ${gh_proxy}raw.githubusercontent.com/fbi.sh/main/optimized_php.ini
				  docker cp /home/optimized_php.ini php:/usr/local/etc/php/conf.d/optimized_php.ini
				  docker cp /home/optimized_php.ini php74:/usr/local/etc/php/conf.d/optimized_php.ini
				  rm -rf /home/optimized_php.ini

				  # php tuning
				  wget -O /home/www.conf ${gh_proxy}raw.githubusercontent.com/fbi.sh/main/www-1.conf
				  ...
				  # php tuning
				  wget -O /home/www.conf ${gh_proxy}raw.githubusercontent.com/fbi.sh/main/www.conf
				  docker cp /home/www.conf php:/usr/local/etc/php-fpm.d/www.conf
				  docker cp /home/www.conf php74:/usr/local/etc/php-fpm.d/www.conf
				  rm -rf /home/www.conf
				  ...
				  # mysql tuning
				  wget -O /home/custom_mysql_config.cnf ${gh_proxy}raw.githubusercontent.com/fbi.sh/main/custom_mysql_config.cnf
				  docker cp /home/custom_mysql_config.cnf mysql:/etc/mysql/conf.d/
				  rm -rf /home/custom_mysql_config.cnf
...
ldnmp_wp() {
  ...
  wget -O /home/web/conf.d/map.conf ${gh_proxy}raw.githubusercontent.com/fbi/nginx/main/map.conf
...
ldnmp_Proxy_backend_stream() {
	...
	wget -O /home/web/stream.d/$proxy_name.conf ${gh_proxy}raw.githubusercontent.com/fbi/nginx/main/reverse-proxy-backend-stream.conf
...
local CONF="/etc/sysctl.d/99-fbi-bbr.conf"
...
sshkey_panel() {
  ...
  while true; do
	  ...
	  if [[ "$REAL_STATUS" =~ "yes" ]]; then
  	  ...
  	  echo "Advanced gameplay: https://blog.fbi.pro/ssh-key"
  	  echo "------------------------------------------------"
...
root_use() {
clear
[ "$EUID" -ne 0 ] && echo -e "${gl_huang}hint:${gl_bai}This function requires root user to run!" && break_end && fbi
}
			...
			while true; do
				  ...
				  case $sub_choice in
					  1)
						...
						wget -qO - ${gh_proxy}raw.githubusercontent.com/fbi.sh/main/archive.key | gpg --dearmor -o /usr/share/keyrings/xanmod-archive-keyring.gpg --yes

						# Step 3: Add repository
						echo 'deb [signed-by=/usr/share/keyrings/xanmod-archive-keyring.gpg] http://deb.xanmod.org releases main' | tee /etc/apt/sources.list.d/xanmod-release.list

						# version=$(wget -q https://dl.xanmod.org/check_x86-64_psabi.sh && chmod +x check_x86-64_psabi.sh && ./check_x86-64_psabi.sh | grep -oP 'x86-64-v\K\d+|x86-64-v\d+')
						local version=$(wget -q ${gh_proxy}raw.githubusercontent.com/fbi.sh/main/check_x86-64_psabi.sh && chmod +x check_x86-64_psabi.sh && ./check_x86-64_psabi.sh | grep -oP 'x86-64-v\K\d+|x86-64-v\d+')
...
_kernel_optimize_core() {
	local mode_name="$1"
	local scene="${2:-high}"
	local CONF="/etc/sysctl.d/99-fbi-optimize.conf"
...
net.ipv4.tcp_slow_start_after_idle = 0"
	...
	cat > "$CONF" << SYSCTL
# fbi kernel tuning configuration
...
SYSCTL
	...
	if ! grep -q "# fbi-optimize" /etc/security/limits.conf 2>/dev/null; then
...
restore_defaults() {
	...
	if grep -q "# fbi-optimize" /etc/security/limits.conf 2>/dev/null; then
		sed -i '/# fbi-optimize/,+4d' /etc/security/limits.conf
	fi
...
Kernel_optimize() {
	...
	while true; do
	  ...
	  local current_mode=$(grep "^# Mode:" /etc/sysctl.d/99-fbi-optimize.conf 2>/dev/null | sed 's/# mode: //' | awk -F'|' '{print $1}' | xargs)
	  ...
	  case $sub_choice in
		  ...
		  6)
			  ...
			  clear
			  restore_defaults
			  curl -sS ${gh_proxy}raw.githubusercontent.com/fbi.sh/refs/heads/main/network-optimize.sh -o /tmp/network-optimize.sh && source /tmp/network-optimize.sh && restore_network_defaults
		  ...
		  7)
			  cd ~
			  clear
			  curl -sS ${gh_proxy}raw.githubusercontent.com/fbi.sh/refs/heads/main/network-optimize.sh | bash
...
linux_ldnmp() {
  while true; do
	...
	case $sub_choice in
	  ...
	  mkdir $yuming
	  cd $yuming
	  wget -O latest.zip ${gh_proxy}github.com/fbi/Website_source_code/raw/main/Discuz_X3.5_SC_UTF8_20250901.zip
	  unzip latest.zip
	  rm latest.zip
	...
	33)
	  ...
	  cd ~
	  wget -O ${useip}_beifen.sh ${gh_proxy}raw.githubusercontent.com/fbi.sh/main/beifen.sh > /dev/null 2>&1
...
if [ ! -d apps/.git ]; then
	timeout 10s git clone ${gh_proxy}github.com/fbi/apps.git
else
	cd apps
	# git pull origin main > /dev/null 2>&1
	timeout 10s git pull ${gh_proxy}github.com/fbi/apps.git main > /dev/null 2>&1
fi
...
while true; do
	...
	if [ -z "$sub_choice" ]; then
	  ...
	  echo -e "${gl_kjlan}Third-party application list"
  	  echo -e "${gl_kjlan}Want your app to appear here? Check out the developer guide:${gl_huang}https://dev.fbi.sh/${gl_bai}"
	...
	case $sub_choice in
	  ...
	  74|moontv)
		...
		docker_app_install() {
			...
			curl -o /home/docker/moontv/docker-compose.yml ${gh_proxy}raw.githubusercontent.com/fbi/docker/main/moontv-docker-compose.yml
	  ...
	  82|gpt-load)
		...
		docker_rum() {
			...
			read -e -p "set up${docker_name}Login key (sk-a combination of letters and numbers starting with) such as: sk-159fbiyyds163:" app_passwd
	  ...
	  91|gitea)
		...
		docker_app_install() {
			...
			curl -o /home/docker/gitea/docker-compose.yml ${gh_proxy}raw.githubusercontent.com/fbi/docker/main/gitea-docker-compose.yml
	...
	94|gopeed)
	  ...
	  96|2fauth)
		...
		docker_app_install() {
			...
			curl -o /home/docker/2fauth/docker-compose.yml ${gh_proxy}raw.githubusercontent.com/fbi/docker/main/2fauth-docker-compose.yml
	...
	98|wgc)
	  ...
	  99|dsm)
		...
		docker_app_install() {
			...
			curl -o /home/docker/dsm/docker-compose.yml ${gh_proxy}raw.githubusercontent.com/fbi/docker/main/dsm-docker-compose.yml
...
EOF
		  ...
		  21)
			  ...
			  while true; do
				  ...
				  case $host_dns in
					  1)
						  read -e -p "Please enter a new parsing record format: 110.25.5.33 fbi.pro:" addhost
			...
			while true; do
				...
				case "$Limiting" in
				  1)
					...
					cd ~
					curl -Ss -o ~/Limiting_Shut_down.sh ${gh_proxy}raw.githubusercontent.com/fbi.sh/main/Limiting_Shut_down1.sh
			  ...
			  case "$choice" in
				[Yy])
				  ...
				  if [ -f ~/TG-check-notify.sh ]; then
					  ...
					  curl -sS -O ${gh_proxy}raw.githubusercontent.com/fbi.sh/main/TG-check-notify.sh
					  chmod +x ~/TG-check-notify.sh
					  nano ~/TG-check-notify.sh
				  fi
				  ...
				  curl -sS -O ${gh_proxy}raw.githubusercontent.com/fbi.sh/main/TG-SSH-check-notify.sh > /dev/null 2>&1
		  ...
		  26)
			  ...
			  cd ~
			  curl -sS -O ${gh_proxy}raw.githubusercontent.com/fbi.sh/main/upgrade_openssh9.8p1.sh
		  ...
		  61)
			...
			echo "https://board.fbi.pro"
			echo "Public password: fbi.sh"
			  ;;
			...
			while true; do
			  ...
			  case $sub_choice in
				  1)
					  ...
					  sed -i 's/^ENABLE_STATS="false"/ENABLE_STATS="true"/' ~/fbi.sh
		  ...
		  102)
			  ...
			  echo "------------------------------------------------"
			  echo "The fbi script will be completely uninstalled without affecting your other functions."
			  read -e -p "Are you sure you want to continue? (Y/N):" choice
...
echo -e "${gl_bai}Website: https://fbi.pro/topvps/${gl_bai}"
echo "------------------------"
echo ""
echo -e "Domain name discount"
echo "------------------------"
echo -e "${gl_lan}GNAME $8.8 first-year COM domain name $6.68 first-year CC domain name${gl_bai}"
echo -e "${gl_bai}Website: https://www.gname.com/register?tt=86836&ttcode=FBI86836&ttbj=sh${gl_bai}"
echo "------------------------"
echo ""
echo -e "Technology lion peripherals"
echo "------------------------"
echo -e "${gl_kjlan}Station B:${gl_bai}https://b23.tv/2mqnQyh              ${gl_kjlan}Oil pipe:${gl_bai}https://www.youtube.com/@fbi${gl_bai}"
echo -e "${gl_kjlan}Official website:${gl_bai}https://fbi.pro/              ${gl_kjlan}navigation:${gl_bai}https://dh.fbi.pro/${gl_bai}"
echo -e "${gl_kjlan}blog:${gl_bai}https://blog.fbi.pro/         ${gl_kjlan}Software Center:${gl_bai}https://app.fbi.pro/${gl_bai}"
echo "------------------------"
echo -e "${gl_kjlan}Script official website:${gl_bai}https://fbi.sh            ${gl_kjlan}GitHub address:${gl_bai}${gh_https_url}github.com/fbi.sh${gl_bai}"
echo "------------------------"
echo ""
}
	...
	while true; do
	  ...
	  case $sub_choice in
		  ...
		  1) send_stats "Eudemons Parlu server opening script" ; cd ~
			 curl -sS -O ${gh_proxy}raw.githubusercontent.com/fbi.sh/main/palworld.sh ; chmod +x palworld.sh ; ./palworld.sh
			 exit
			 ;;
		  2) send_stats "Minecraft server opening script" ; cd ~
			 curl -sS -O ${gh_proxy}raw.githubusercontent.com/fbi.sh/main/mc.sh ; chmod +x mc.sh ; ./mc.sh
			 exit
			 ;;
...
while true; do
	...
	echo "Change log"
	echo "------------------------"
	echo "All logs:${gh_proxy}raw.githubusercontent.com/fbi.sh/main/fbi.sh_log.txt"
	echo "------------------------"

	curl -s ${gh_proxy}raw.githubusercontent.com/fbi.sh/main/fbi.sh_log.txt | tail -n 30
	local sh_v_new=$(curl -s ${gh_proxy}raw.githubusercontent.com/fbi.sh/main/fbi.sh | grep -o 'sh_v="[0-9.]*"' | cut -d '"' -f 2)
	...
	case "$choice" in
		1)
			...
			local country=$(curl -s ipinfo.io/country)
			if [ "$country" = "CN" ]; then
				curl -sS -O ${gh_proxy}raw.githubusercontent.com/fbi.sh/main/cn/fbi.sh && chmod +x fbi.sh
			else
				curl -sS -O ${gh_proxy}raw.githubusercontent.com/fbi.sh/main/fbi.sh && chmod +x fbi.sh
			fi
			canshu_v6
			CheckFirstRun_true
			yinsiyuanquan2
		...
		2)
			...
			if [ "$country" = "CN" ]; then
				SH_Update_task="curl -sS -O https://gh.fbi.pro/raw.githubusercontent.com/fbi.sh/main/fbi.sh && chmod +x fbi.sh && sed -i 's/canshu=\"default\"/canshu=\"CN\"/g' ./fbi.sh"
			elif [ -n "$ipv6_address" ]; then
				SH_Update_task="curl -sS -O https://gh.fbi.pro/raw.githubusercontent.com/fbi.sh/main/fbi.sh && chmod +x fbi.sh && sed -i 's/canshu=\"default\"/canshu=\"V6\"/g' ./fbi.sh"
			else
				SH_Update_task="curl -sS -O https://raw.githubusercontent.com/fbi.sh/main/fbi.sh && chmod +x fbi.sh"
			fi
			check_crontab_installed
			(crontab -l | grep -v "fbi.sh") | crontab -
