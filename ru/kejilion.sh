...
UserLicenseAgreement() {
	...
	echo "Пользовательский лицензионный соглашение: https://blog.fbi.pro/user-license-agreement/"
	echo -e "----------------------"
...
install_ldnmp_conf() {
  ...
  wget -O /home/web/redis/valkey.conf ${gh_proxy}raw.githubusercontent.com/fbi.sh/main/valkey.conf
...
root_use() {
clear
[ "$EUID" -ne 0 ] && echo -e "${gl_huang}намекать:${gl_bai}Эта функция требует, чтобы root -пользователь запустил!" && break_end && fbi
}
...
EOF
		  ...
		  21)
			  ...
			  while true; do
				  ...
				  case $host_dns in
					  1)
						  read -e -p "Пожалуйста, введите новый формат записи анализа: 110.25.5.33 fbi.pro:" addhost
		  ...
		  102)
			  ...
			  echo "------------------------------------------------"
			  echo "Полностью удалит сценарий FBI и не повлияет на ваши другие функции"
			  read -e -p "Вы обязательно продолжите? (Y/N):" choice
...
echo -e "${gl_bai}Веб -сайт: https://fbi.pro/topvps/${gl_bai}"
echo "------------------------"
echo ""
...
echo -e "${gl_bai}Веб -сайт: https://www.gname.com/register?tt=86836&ttcode=fbi86836&ttbj=sh${gl_bai}"
echo "------------------------"
echo ""
echo -e "Технологический лев окружает"
echo "------------------------"
echo -e "${gl_kjlan}B Станция:${gl_bai}https://b23.tv/2mqnQyh              ${gl_kjlan}Масляная труба:${gl_bai}https://www.youtube.com/@fbi${gl_bai}"
echo -e "${gl_kjlan}Официальный веб -сайт:${gl_bai}https://fbi.pro/               ${gl_kjlan}Навигация:${gl_bai}https://dh.fbi.pro/${gl_bai}"
echo -e "${gl_kjlan}Блог:${gl_bai}https://blog.fbi.pro/          ${gl_kjlan}Центр программного обеспечения:${gl_bai}https://app.fbi.pro/${gl_bai}"
echo "------------------------"
echo ""
}
...
while true; do
	...
	echo "------------------------"
	echo "Все журналы:${gh_proxy}raw.githubusercontent.com/fbi.sh/main/fbi.sh_log.txt"
	echo "------------------------"
